...
# 【修复1】确保这里的文件名和类名对应。如果文件名带 D，请写成 nets.PLRQuant
from nets.PLRQuant import UCTransNet
...
# 【修复2】改用升级后的损失函数类名
from utils import CosineAnnealingWarmRestarts, WeightedDiceBCE_clDice, MultiTaskOcuLoss
...
def main_loop(batch_size=config.batch_size, model_type='', tensorboard=True):
    ...
    val_loader = DataLoader(val_dataset, batch_size=config.batch_size, shuffle=True, worker_init_fn=worker_init_fn,
                            num_workers=8, pin_memory=True)

    lr = config.learning_rate
    logger.info(model_type)

    # 【创新点激活】使用带 clDice 的损失函数
    if config.n_labels == 1:
        # 任务 0 (瞳孔) 或 任务 2 (OCTA血管)
        logger.info("Using Binary Loss: WeightedDiceBCE_clDice")
        criterion = WeightedDiceBCE_clDice(
        ...
        # 任务 1 (OCT 10类层级分割)
        logger.info(f"Using Multi-Class Loss: MultiTaskOcuLoss for {config.n_labels} classes")
        criterion = MultiTaskOcuLoss(n_classes=config.n_labels)

    if model_type == 'PLRQuant' or model_type == 'PLRQuant_pretrain':
        config_vit = config.get_CTranS_config()
        logger.info('transformer head num: {}'.format(config_vit.transformer.num_heads))
        model = UCTransNet(config_vit, n_channels=config.n_channels, n_classes=config.n_labels)

        if model_type == 'PLRQuant_pretrain':
            pretrained_UNet_model_path = "./nets/best_model-UNet.pth.tar"
            if os.path.exists(pretrained_UNet_model_path):
                pretrained_UNet = torch.load(pretrained_UNet_model_path, map_location='cuda')
                model.load_state_dict(pretrained_UNet['state_dict'], strict=False)
